...
import { Search, ShoppingCart, Heart, User, Menu, X, Mail, Lock, Eye, EyeOff } from 'lucide-react'

export function Header() {
  const { user, loginWithGoogle, loginWithFacebook, loginWithEmail, signupWithEmail, logout, isAuthenticated } = useAuth()
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      setAuthError('পাসওয়ার্ড মিলছে না!')
      return
    }
    ...
    try {
      ...
      if (success) {
        setShowAuthModal(false)
        resetAuth()
        alert('✅ অ্যাকাউন্ট তৈরি হয়েছে! ইমেইল যাচাই করুন।')
      } else {
            ...
            <div className="p-6 space-y-4">
              ...
              {authMode === 'login' ? (
                ...
                <form onSubmit={handleSignup} className="space-y-3">
                  ...
                  <button type="submit" disabled={authLoading}
                    className="w-full bg-[#1877F2] hover:bg-blue-700 text-white py-3 rounded-lg font-medium disabled:opacity-50 bengali-text">
                    {authLoading ? 'তৈরি হচ্ছে...' : 'অ্যাকাউন্ট তৈরি করুন'}
                  </button>
                </form>
              )}
