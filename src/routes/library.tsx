...
function LibraryPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('/api/user-data').then(r => r.ok ? r.json() : { purchasedBooks: [] }),
      fetch('/api/reading-progress').then(r => r.ok ? r.json() : {}).catch(() => ({})),
    ]).then(async ([userData, readingProgress]) => {
      setProgress(readingProgress || {})
      const purchasedIds: string[] = userData?.purchasedBooks || []
      ...
      const bookDetails = await Promise.all(
        purchasedIds.map((id: string) =>
          fetch(`/api/books/detail?id=${id}`)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center p-8">
          ...
          <button onClick={loginWithGoogle} className="bg-[#1877F2] text-white px-6 py-3 rounded-lg bengali-text">
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <Link to="/books" className="bg-[#1877F2] text-white px-6 py-3 rounded-lg inline-block bengali-text">
            বই কিনুন
          </Link>
        </div>
      ) : (
          ...
          {books.map(book => {
            ...
            return (
              <Link
                ...
                className="flex gap-4 bg-white p-4 rounded-xl border hover:shadow-md transition-shadow"
              >
                <div className="w-20 h-28 bg-gradient-to-br from-blue-50 to-indigo-100 rounded-lg flex items-center justify-center shrink-0 overflow-hidden">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2">
                      <div className="flex items-center justify-between text-xs text-gray-500 mb-1">
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {p.lastRead ? new Date(p.lastRead).toLocaleDateString('bn-BD') : ''}
                        </span>
                      </div>
                  ...
                  <span className="inline-block mt-2 text-[#1877F2] text-sm font-medium bengali-text">
                    {p?.progress > 0 ? 'পড়া চালিয়ে যান →' : 'পড়া শুরু করুন →'}
                  </span>
                </div>
              </Link>
            )
