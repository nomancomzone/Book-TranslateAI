...
function WishlistPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/wishlist', { headers: { 'x-user-email': user.email } })
      .then(r => r.ok ? r.json() : [])
  ...
  const addWish = async () => {
    if (!bookName.trim() || !reason.trim()) return
    setSubmitting(true)
    try {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-2 bengali-text">উইশলিস্ট</h1>
      <p className="text-gray-500 mb-6 bengali-text">কোন বই বাংলায় অনুবাদ হোক চান? বইয়ের নাম, লেখক ও কারণ জানান!</p>

      {/* Add form */}
      <div className="bg-white rounded-xl border p-6 mb-8">
        <h3 className="font-bold mb-4 bengali-text flex items-center gap-2">
          <Plus className="w-5 h-5" /> বই অনুবাদের অনুরোধ
        </h3>
        <div className="space-y-3">
          ...
          <input
            ...
            placeholder="লেখকের নাম (ঐচ্ছিক)"
            className="w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-300 bengali-text"
          />
          <textarea
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            placeholder="কেন এই বইটি চান? *"
            className="w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-300 bengali-text"
            rows={3}
          />
          <button
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-400 mt-2 bengali-text">উপরের form থেকে বই অনুরোধ করুন</p>
        </div>
      ) : (
        <div className="space-y-3">
          {wishlist.map(item => (
              ...
              <div>
                ...
                {item.note && (
                  <p className="text-sm text-gray-500 mt-1 bengali-text">
                    <span className="text-gray-400">কারণ:</span> {item.note}
                  </p>
                )}
